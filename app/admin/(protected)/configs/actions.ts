...
function withSourceMessage(message: string, sourceId: string | null) {
  const url = new URL(withMessage("/admin/configs/new", "error", message), "http://localhost")
...
function getConfigsReturnPath(formData: FormData) {
  ...
  if (returnTo?.startsWith("/admin/configs")) {
    return returnTo
  }
...
async function getScopedConfig(user: AppUser, id: string) {
  if (isAdminUser(user)) {
    return getConfig(id)
  }
  ...
  if (!row) return null
  if (row.group_name !== scopeGroup) return null
  return row
}

async function getScopedConfigs(user: AppUser, ids: string[]) {
  const scopeGroup = isAdminUser(user) ? null : getRequiredGroupName(user)
  const rows = await listConfigs(scopeGroup)
...
export async function createConfigAction(formData: FormData) {
  ...
  redirect(withMessage("/admin/configs", "success", "配置已创建"))
}
...
export async function updateConfigAction(formData: FormData) {
  ...
  try {
    ...
    redirect(withMessage(`/admin/configs/${id}`, "error", message))
  }

  revalidatePath("/admin")
  revalidatePath("/admin/configs")
  redirect(withMessage(`/admin/configs/${id}`, "success", "配置已更新"))
}
...
export async function deleteConfigAction(formData: FormData) {
  ...
  revalidatePath("/admin")
  revalidatePath("/admin/configs")
  redirect(withMessage("/admin/configs", "success", "配置已删除"))
}
...
export async function clearConfigHistoryAction(formData: FormData) {
  ...
  try {
    const config = await getScopedConfig(user, id)
...
export async function batchConfigAction(formData: FormData) {
  ...
  try {
    ...
    const selectedConfigs = await getScopedConfigs(user, ids)
    ...
    switch (operation) {
      ...
      case "replace_model": {
        ...
        const selectedTypes = parseProviderTypeSet(
          formData.getAll("selected_types").map((item) => item.toString())
        )
